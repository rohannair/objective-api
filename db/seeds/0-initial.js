...
exports.seed = function(knex, Promise) {
  ...
  let companyMissionid = '2a31d5b5-b6ea-40da-8d78-5db4e1c93482';
  ...
  .then(() => encryptPassword('password123'))
  .then(password =>
    knex('users').insert([
      {
        id: userId,
  ...
  .then(() =>
    knex('missions').insert({
      id: companyMissionid,
      name: 'Increase resources available to product and sales teams to help accomplish bigger goals sooner.',
      duration: '12 months'
    })
  )
  .then(() => Promise.all([
  ...
  .then(() => Promise.all([
    ...
    knex('companies_missions').insert({
      company_id: companyId,
      mission_id: companyMissionid
    })
  ]))
}
